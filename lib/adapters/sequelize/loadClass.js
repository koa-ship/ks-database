...
module.exports = function loadClass(sequelize, name, klass) {
  ...
  let preHooks = hooks.map((item) => { return 'before' + item.charAt(0).toUpperCase() + item.slice(1); });
  let postHooks = hooks.map((item) => { return 'after' + item.charAt(0).toUpperCase() + item.slice(1); });
