...
function wrap(schema, klass) {
  ...
  let preHooks = hooks.map((item) => { return 'pre' + item.charAt(0).toUpperCase() + item.slice(1); });
  let postHooks = hooks.map((item) => { return 'post' + item.charAt(0).toUpperCase() + item.slice(1); });  
